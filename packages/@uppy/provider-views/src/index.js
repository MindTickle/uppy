...
module.exports = class ProviderView {
  ...
  addFolder(folder) {
    ...
    return this.Provider.list(folder.requestPath)
      .then(res => {
        let files = [];
        res.items.forEach(item => {
          if (!item.isFolder) {
    ...
    const handleToken = e => {
      if (
        !this._isOriginAllowed(
          e.origin,
          this.plugin.opts.serverPattern
        ) /*||
        e.source !== authWindow*/
      ) {
        this.plugin.uppy.log(
          `rejecting event from ${e.origin} vs allowed pattern ${this.plugin.opts.serverPattern}`
        );
        return;
      }
      authWindow.close();
  ...
  _isOriginAllowed(origin, allowedOrigin) {
    ...
    return patterns.filter(pattern => pattern !== null).some(pattern => pattern.test(origin));
  }
