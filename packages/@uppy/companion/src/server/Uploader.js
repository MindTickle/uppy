...
class Uploader {
  ...
  constructor(options) {
    ...
    this.writer = fs
      .createWriteStream(this.options.path, { mode: 0o666 }) // no executable files
  ...
  cleanUp() {
    fs.unlink(this.options.path, err => {
      if (err) {
        logger.error(
          `cleanup failed for: ${this.options.path} err: ${err}`,
          "uploader.cleanup.error"
        );
      }
    });
  ...
  handleChunk(chunk) {
    logger.debug(
      `${this.shortToken} ${this.writer.bytesWritten} bytes`,
      "uploader.download.progress"
    );
  ...
  uploadMultipart() {
    ...
    request.post(
      { url: this.options.endpoint, headers, formData, encoding: null },
      (error, response, body) => {
        if (error) {
