...
class MultipartUploader {
  ...
  _initChunks() {
    const chunks = [];
    const chunkSize = Math.max(Math.ceil(this.file.size / 10000), this.chunkSize);
  ...
  _createUpload() {
    return Promise.resolve()
      .then(() => this.options.createMultipartUpload(this.chunks.length))
      .then(result => {
        const valid =
      ...
      .then(result => {
        ...
        this.completeURI = result.complete;
        this.abortURI = result.abort;
  ...
  _uploadPart(index) {
    ...
    return Promise.resolve().then(() => {
      try {
        const url = this.parts[index].url;
